...
def array_to_pointcloud2(cloud_arr: np.ndarray, column_names: Union[List[str], tuple], stamp=None, frame_id=None):
    """
    Converts a numpy record array to a sensor_msgs.msg.PointCloud2.
    """
    assert cloud_arr.shape[1] == len(column_names), 'Mismatch a number of columns and a number of column names'

    cloud_arr = cloud_arr.view([(c_name, cloud_arr.dtype) for c_name in column_names])
    ...
    cloud_msg.height = cloud_arr.shape[1]
